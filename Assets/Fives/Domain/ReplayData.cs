...
namespace Fives.Domain
{
    ...
    public sealed class ReplayData
    {
        ...
        public int Size { get; }
        public int Seed { get; }
        public IReadOnlyList<Swap> Moves { get; }

        public ReplayData(int version, int size, int seed, IReadOnlyList<Swap> moves)
        {
        ...
        {
            var initial = SeededShuffle.Create(Size, Seed);
            var validation = initial.Copy();
            for (var i = 0; i < Moves.Count; i++)
            {
                if (validation.IsSolved || !validation.TrySwap(Moves[i]))
