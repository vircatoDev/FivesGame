...
{
    /// <summary>Pushes the move counter and Undo/Replay availability to the gameplay HUD when they change.</summary>
    public class BoardHudSystem : IEcsRunSystem
    {
        ...
        private readonly EcsFilter<BoardReplayComponent> _replays = null;
        ...
        public void Run()
        {
            ...
            var moves = history.Moves.Count;
            var replaying = _replays.GetEntitiesCount() > 0;
            var ready = _session.IsRunning && !_session.IsCompleted;
            var canUndo = ready && !replaying && _moves.GetEntitiesCount() == 0 && moves > 0;
            var hud = new BoardHud(history.Seed, moves, replaying ? _replays.Get1(0).Position : 0,
                replaying, canUndo, canUndo || (ready && replaying));
